...
class ConfigBottomSheetDialogFragment : BottomSheetDialogFragment() {
    ...
    private var timeMillisHorizontal: Long = 0L

    private var _bindings: ViewConfigBinding? = null
    private val binding get() = _bindings!!

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        _bindings = ViewConfigBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    private fun inflateView() {
        ...
        binding.viewConfigIbDayMode.setOnClickListener {
            if (isNightMode){
                ...
                if (currentTimeMillis-timeMillis<3000){
                    UiUtil.getSingletonToast(context,"操作频繁，请稍候...").show()
                }else{
...
//                    toast("操作频繁，请稍候...")
        ...
        binding.buttonVertical.setOnClickListener {
            ...
            if (config.direction!=Config.Direction.VERTICAL){
                val currentTimeMillis = System.currentTimeMillis()
                if (currentTimeMillis-timeMillis<3000){
                    UiUtil.getSingletonToast(context,"操作频繁，请稍候...").show()
                }else{
